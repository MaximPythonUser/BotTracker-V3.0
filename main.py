...
scheduler = AsyncIOScheduler(jobstores=jobstore, timezone=moscow_tz)
...
async def send_reminder(user_id: int, training_name: str, date: str, time: str):
    print(f"🔔 ОТПРАВЛЕНО НАПОМИНАНИЕ: Тренировка '{training_name}' ({date} {time}) для пользователя {user_id}")
    try:
    ...
    async def schedule_existing_reminders():
        ...
        for row in rows:
            ...
            try:
                ...
                print(f"⚠️ Пропущена тренировка с некорректной датой/временем: ID {row['id']}")
                continue

            # Если тренировка уже прошла (плюс час на проверку статуса), не планируем
            ...
            if reminder_time > now_dt:
                ...
                if not scheduler.get_job(job_id):
                    scheduler.add_job(
                        ...
                        args=[row["user_id"], row["training_name"], row["date"], row["start_time"]],
                        id=job_id,
                        replace_existing=True
                    )
                    print(f"✅ Восстановлено напоминание для ID {training_id} на {reminder_time}")
            ...
            if check_time > now_dt:
                ...
                if not scheduler.get_job(job_id):
                    ...
                    print(f"✅ Восстановлена проверка статуса для ID {training_id} на {check_time}")

        print(f"✅ Восстановлено расписание для {count} активных тренировок (напоминания и проверки).")

async def check_training_status(training_id: int, user_id: int, name: str, date: str, time: str):
    print(f"⏰ Время проверки статуса для тренировки ID {training_id}")
...
reply_markup=get_post_trainings_keyboard(training_id)
        ...
        print(f"❌ Не удалось отправить запрос о статусе пользователю {user_id}: {e}")
...
async def cmd_add(message: types.Message):
    ...
    train_datetime_str = f"{date} {time}"
    train_datetime = datetime.strptime(train_datetime_str, "%Y-%m-%d %H:%M")
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.commit()
        training_id = cursor.lastrowid  # Получаем уникальный ID
       # Планируем НАПОМИНАНИЕ
    reminder_time = train_datetime - timedelta(minutes=20)
    reminder_job_id = f"reminder_{training_id}"
    scheduler.add_job(
        send_reminder,
        trigger="date",
        run_date=reminder_time,
        args=[user_id, name, date, time],
        id=reminder_job_id,
        replace_existing=True
    )
    print(f"✅ Напоминание создано для ID {training_id} на {reminder_time}")

    check_time = train_datetime + timedelta(hours=1)
    check_job_id = f"check_{training_id}"
    scheduler.add_job(
        check_training_status,
        trigger="date",
        run_date=check_time,
        args=[training_id, user_id, name, date, time],
        id=check_job_id,
        replace_existing=True
    )
    print(f"✅ Проверка статуса создана для ID {training_id} на {check_time}")

    await message.answer(f"✅ Тренировка сохранена: {name} на {date} в {time}.\n"
        f"Я напомню за 20 минут и спрошу о результате через час после начала.")
            ...
            for job_id in job_ids:
                try:
                    if scheduler.get_job(job_id):
                        ...
                        print(f"⚠️ Задача {job_id} не найдена в планировщике (это нормально, если тренировка уже прошла).")
...
async def process_post_training_callback(callback_query: types.CallbackQuery):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        cursor = await db.execute("SELECT * FROM one_off_trainings WHERE id = ?", (training_id,))
    ...
    try:
        ...
        print(f"⚠️ Задача для ID {training_id} уже отсутствовала: {e}")
...
async def process_edit_time(message: types.Message, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        old_job_id = f"reminder_{target_id}"
        try:
            if scheduler.get_job(old_job_id):
                scheduler.remove_job(old_job_id, jobstore="default")
                print(f"🗑️ Удалено старое напоминание для ID {target_id}")
        except Exception as e:
        ...
        if train_datetime > now_dt:
            reminder_time = train_datetime - timedelta(minutes=20)
            new_job_id = f"reminder_{target_id}"
            scheduler.add_job(
                send_reminder,
                trigger="date",
                run_date=reminder_time,
                args=[user_id, name, old_date, new_time],
                id=new_job_id,
                replace_existing=True
            )
            print(f"✅ Запланировано новое напоминание для ID {target_id} на {reminder_time}")
